...
if zijd != 'n':
    if len(NRMatAFx) != 0:
        ...
        Plot_Zijderveld(NRMatAFx, NRMatAFy, NRMatAFz, NRMatAFstep,unit=unit,title='NRM@AF',color='AF') #AF
        if save == 'y':
            id = type_of_file.index('NRM')
            plt.savefig(path + 'Plots/' + sample_name[id] + '-ZIJD.pdf', format='pdf', dpi=200, bbox_inches="tight")
        plt.figure(figsize=(5, 5))
        Plot_Zijderveld(NRMatAFx, NRMatAFy, NRMatAFz, NRMatAFstep, unit=unit, title='NRM@AF', gui='guiX', color='k')#X ou Z
    if len(ARMatAFx) != 0:
        plt.figure(figsize=(5, 5))
