...
def Merge_AF_demag(NRMx,NRMy,NRMz,NRMAF,Mx,My,Mz,MAF):

    AF = sort(list(set(NRMAF) & set(MAF)))

    NRMx = [NRMx[k] for k in np.arange(len(NRMx)) if NRMAF[k] in AF]
    NRMy = [NRMy[k] for k in np.arange(len(NRMy)) if NRMAF[k] in AF]
    NRMz = [NRMz[k] for k in np.arange(len(NRMz)) if NRMAF[k] in AF]
    Mx = [Mx[k] for k in np.arange(len(Mx)) if MAF[k] in AF]
    My = [My[k] for k in np.arange(len(My)) if MAF[k] in AF]
    Mz = [Mz[k] for k in np.arange(len(Mz)) if MAF[k] in AF]

    return NRMx,NRMy,NRMz,Mx,My,Mz,AF


def Plot_AF_demag(Mx, My, Mz, step, quantity, color='k', marker='o', path=''):

    Mx, My, Mz = np.array(Mx), np.array(My), np.array(Mz)
    M = np.sqrt(Mx ** 2 + My ** 2 + Mz ** 2)
    Mlost = np.sqrt((Mx-Mx[0]) ** 2 + (My-My[0]) ** 2 + (Mz-Mz[0]) ** 2)

    plt.xlabel('AF level (mT)', fontsize=13)
    plt.xlim(-2, max(step) + 2)
    # plt.ylabel('1-Mlost/NRM0')
    # plt.plot(step, 1-Mlost/M[0], color='k',marker=marker, markeredgecolor='k', markerfacecolor=color,ms=5, lw=0.5, label=quantity)
    plt.ylim(0,1.1)
    plt.xlim(0, 160)
    plt.ylabel('Normalized moment', fontsize=13)
    plt.plot(step, M / M[0], c='k', marker=marker, mec='k', mfc=color, ms=8, mew=0.5, lw=0.5, label=quantity)
    plt.xticks(fontsize=13)
    plt.yticks(fontsize=13)
    plt.legend()


def Plot_thermal_demag(Mx, My, Mz, step, norm=False, color='k', line=0.5, marker='o', path=''):
    ...
    plt.xlim(0,750)
    if norm == True:
        plt.ylabel('Normalized moment M/M0')
        plt.ylim(0,1)
    else:
        plt.ylabel('Moment (A m2)')
        plt.ylim(0, np.max(NRM)+0.1*np.max(NRM))
    plt.plot(step,NRM,color=color,marker=marker,markeredgecolor='k',markerfacecolor='k',ms=8,lw=line)
    fig.tight_layout()



def Plot_thellier_intensities(Mx, My, Mz, Thstep, type):
    ...
    fig = plt.figure()
    plt.xlabel('Temperature (C)')
    plt.ylabel('Moment normalized to NRM0')
    plt.xlim(10, max(zStep) + 10)
    plt.ylim(0,1.1)
    plt.plot(zStep,NRM/NRM[0],color='k',marker='o',ms=8,mfc='darkred',mec='k',mew=0.5,lw=0.5,label='NRM remaining')
...
def Plot_aray(Mx, My, Mz, Thstep, type):
    ...
    plt.xlim(-0.02, 1.05)
    plt.ylim(-0.02, 1.05)
    plt.plot(pTRMgained / NRM[0], NRM / NRM[0], color='k', marker='o', mfc='darkred', mec='k', mew=0.5,
             ms=8, lw=0.5)
...
def Calc_stat_thellier(NRM,pTRMgained,cHgained,zStep,cStep,field):
