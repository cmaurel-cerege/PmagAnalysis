...
def BPCA(y):
    ...
    for i in range(20):
        ...
        w_mean, w_cov = BPCA_update_w(y, q, x_mean, x_cov, mu_mean, gamma_a, gamma_b, alpha_a,
                                      alpha_b)  # update w distribution
        ...
        gamma_a, gamma_b = BPCA_update_gamma(y, q, mu_mean, w_mean, x_mean, H_gamma_a,
                                             H_gamma_b)  # update gamma distribution
...
def BPCA_postprocess(y):
    ...
    mod = opt.minimize(lambda theta: (oaG_cdf(theta, zeta[0][0]) - 0.95) ** 2, 1)  # find 95th percentile in cdf
